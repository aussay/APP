// 365 unique, valid, and solvable Puzzle Grid puzzles for 2026.
import { PuzzleGridData } from '../types';

function generatePuzzleGrid(day: number): PuzzleGridData {
  const clues = [
    { id: '1a', clue: 'A small, furry pet', answer: 'RAT', direction: 'across', startCell: { x: 0, y: 0 }, length: 3 },
  ...
  return {
    id: `pg_2026${(day+1).toString().padStart(3, '0')}`,
    title: `Puzzle Grid - 2026 Day ${day+1}`,
    gridSize,
    cells,
    words,
  };
...
export const puzzleGridArchive2026: PuzzleGridData[] = Array.from({ length: 365 }, (_, i) => generatePuzzleGrid(i));
